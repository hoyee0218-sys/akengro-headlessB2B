...
function SearchResultsPredictiveQueries({
  ...
  return (
    <div className="predictive-search-result predictive-search-result--queries" key="queries">
      <h5>{t('search.queries')}</h5>
      <ul>
        {queries.map((suggestion) => {
          ...
          return (
            <li className="predictive-search-result-item" key={suggestion.text}>
              <Link onClick={closeSearch} to={href} prefetch="intent">
                <div>
                  <p>{suggestion.text}</p>
                </div>
              </Link>
...
function SearchResultsPredictiveEmpty({
  ...
  return <p>{t('search.noResultsFor', {term: term.current})}</p>;
}
